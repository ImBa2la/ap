...
function importSettings(DOMElement $e){
	...
	$attrs = array('title','add','delete','nocheckbox','prefix');
	...
	foreach($res as $btn){
		if($btn->hasAttribute('action')){
			$buttons[$btn->getAttribute('action')]['title'] = $btn->getAttribute('title');
			$buttons[$btn->getAttribute('action')]['class'] = $btn->getAttribute('class');
		}
	}
...
function addRow($id,$values = null,$buttons = null){
	...
	if($b = $this->getButtons()) foreach($b as $action => $data) $row->setButton($action,$data['title'],$data['class']);
	return $row;
}
function getRow($id){
...
function __construct(DOMElement $e,$headers,$values = null){
	...
	if(isset($values['buttons']) && is_array($values['buttons']))
		foreach($values['buttons'] as $action => $data)
			$this->setButton($action,$data['title'],$data['class']);
}
...
function setButton($action,$title,$class=false){
	if($b = $this->getButtons()){
		if($e = $b->getById($action,'action'))
			$e->parentNode->removeChild($e);
		$b->append(array('action'=>$action,'title'=>$title,'class'=>$class));
	}
}
}
?>