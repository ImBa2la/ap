...
function hasCaptcha(){
	...
	return $xml->evaluate('count(.//field[@type="captcha"])',$this->form);
}
function isSent(){
	$xml = new xml($this->form);
...
function getSentData(){	
	...
	foreach($res as $field){
		...
		if($field->hasAttribute('mail')){
			...
			$arRes['xml']->de()->appendChild($arRes['xml']->createElement($field->getAttribute('name'),array('label'=>$f['label']),$f['value']));
		}
		if($field->hasAttribute('mysql')){
			$arRes['mysql'][$field->getAttribute('mysql')] = '"'.$f['value'].'"';
		}
	}
	return $arRes;
}
function sendEmail($xml){
	...
	if($xml->query('//field')->item(0)
		...
		&& ($mail = new mymail('no-reply@'.$domain,$email,'Новое сообщение от пользователя с сайта - '.$domain,$content)) //объект для отправки письма		
	...
	)xml::setElementText($e,xml::getElementText ($this->xml->query('good',$this->form_xml)->item(0)));
	else xml::setElementText($e,xml::getElementText ($this->xml->query('fail',$this->form_xml)->item(0)));
...
function sendEmailUser($xml){
	if(($tpl = new template(
		...
		&& ($mail = new mymail('no-reply@'.$domain,$email,'Вами было отправлено сообщение с сайта - '.$domain,$content)) //объект для отправки письма		
	...
	if($this->form = $this->query('form')->item(0)){//нашли форму
		...
		if($this->isSent()){ //форму отправили
			if(!$this->check() && ($res = $this->getSentData())){
				if($res['xml']) $this->sendEmail($res['xml']);
				if(count($res['mysql']) > 0) vdump("SEND EMAIL: LINE ".__LINE__.", feedback.php");
